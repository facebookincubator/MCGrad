...
class MulticalibrationError:
    ...
    def profile_time_memory(self) -> dict[str, float]:
        """
        Profiles the time and memory usage of the metric computation.
        """

        logger.info("Profiling time and memory usage of the metric computation.")
        tracemalloc.start()
        start_time = time.time()
        _ = self.segments[0]
        segment_time = time.time() - start_time
        segment_memory = tracemalloc.get_traced_memory()
        _ = self.segment_ecces_absolute
        segment_ecces_absolute_time = time.time() - start_time - segment_time
        segment_ecces_absolute_memory = tracemalloc.get_traced_memory()
        _ = self.segment_sigmas
        segment_segment_sigmas_time = (
            time.time() - start_time - segment_time - segment_ecces_absolute_time
        )
        segment_segment_sigmas_memory = tracemalloc.get_traced_memory()
        _ = self.segment_ecces_sigma_scale
        segment_ecces_sigma_scale_time = (
            time.time()
            - start_time
            - segment_time
            - segment_ecces_absolute_time
            - segment_segment_sigmas_time
        )
        segment_ecces_sigma_scale_memory = tracemalloc.get_traced_memory()
        mce_sigma_scale = self.mce_sigma_scale
        segment_mce_sigma_scale_time = (
            time.time()
            - start_time
            - segment_time
            - segment_ecces_absolute_time
        ...
        segment_mce_sigma_scale_memory = tracemalloc.get_traced_memory()
        # stopping the library
        tracemalloc.stop()
        results_dictionary = {
            "segment_time": segment_time,
            "segment_memory_curr": segment_memory[0] / 1024**2,
            "segment_memory_peak": segment_memory[1] / 1024**2,
            "segment_ecces_absolute_time": segment_ecces_absolute_time,
            "segment_ecces_absolute_memory_curr": segment_ecces_absolute_memory[0]
            / 1024**2,
            "segment_ecces_absolute_memory_peak": segment_ecces_absolute_memory[1]
            / 1024**2,
            "segment_segment_sigmas_time": segment_segment_sigmas_time,
            "segment_segment_sigmas_memory_curr": segment_segment_sigmas_memory[0]
            / 1024**2,
            "segment_segment_sigmas_memory_peak": segment_segment_sigmas_memory[1]
            / 1024**2,
            "segment_ecces_sigma_scale_time": segment_ecces_sigma_scale_time,
            "segment_ecces_sigma_scale_memory_curr": segment_ecces_sigma_scale_memory[0]
            / 1024**2,
            "segment_ecces_sigma_scale_memory_peak": segment_ecces_sigma_scale_memory[1]
            / 1024**2,
            "segment_mce_sigma_scale_time": segment_mce_sigma_scale_time,
            "segment_mce_sigma_scale_memory_curr": segment_mce_sigma_scale_memory[0]
            / 1024**2,
            "segment_mce_sigma_scale_memory_peak": segment_mce_sigma_scale_memory[1]
            / 1024**2,
            "mce_sigma_scale": mce_sigma_scale,
        }
