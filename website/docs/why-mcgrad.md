...
**Multicalibration** — calibration across all relevant groups (sometimes referred as *segments*) — is essential for ML systems that make decisions based on predicted probabilities. Whether in content ranking, recommender systems, digital advertising, or content moderation, it is not enough for predictions to be well-calibrated globally; they must be calibrated for every user segment, content type, and other dimensions.

Despite its importance, multicalibration has seen limited industry adoption due to three core challenges:

**1. Manual Group Specification.** Existing multicalibration methods require you to manually define “protected groups” — not just which features matter, but explicit segment indicators (e.g., "is an adult in the US?"). This is impractical because:

- Practitioners may not know all relevant groups in advance.
- Group definitions change over time (legal, policy, ethical).
- Many teams simply want better calibration and performance, not group engineering.

**2. Lack of Scalability.** Existing methods don't scale to large datasets or many protected groups. For example, some algorithms scale linearly (or worse) in time and memory with the number of segments, becoming infeasible to productionize at web scale, where thousands of features and billions of samples can define a huge number of segments.

**3. Risk of Harming Performance.** Existing methods lack guardrails for safe deployment. Being a post-processing method, the risk that multicalibration might *harm* model performance (e.g., through overfitting) prevents adoption.


### MCGrad: Designed for Real-World Impact

MCGrad is a novel, scalable multicalibration algorithm that overcomes these barriers:
- **Pass features, not groups**: MCGrad only requires specification of a set of *protected features* (rather than groups). It then automatically identifies miscalibrated regions within this feature space, calibrating predictions across all segments that can be defined from these features.
...
- **Safe by design**: MCGrad is a *likelihood-improving procedure* — it can only improve model performance on training data. Combined with early stopping, this ensures that model performance is not harmed.
...
- [Installation](installation.md) - Install MCGrad + dependencies.
